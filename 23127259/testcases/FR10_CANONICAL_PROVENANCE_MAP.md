...
| FR10-AI-006 | Valid Admin Cancellation on Pending Order | System Administrator | pending | PUT | `/api/admin/orders/:id/status` | `{ "status": "canceled" }` | Order is successfully canceled by Admin | `200 OK` | **VALID AS-IS** |
| FR10-AI-007 | Valid Customer Self-Cancellation on Confirmed Order | Order Owner Customer | confirmed | PUT | `/api/orders/:id/cancel` | `Empty` | Customer successfully cancels a confirmed order | `200 OK` | **VALID AS-IS** |
