...
def main():
    print("=== RUNNING FR-10 DEEP STATIC VALIDATOR GATE ===")
    ...
    # 5. Extract & Count Request Definitions and Script-Level Calls
    ...
    # 8. Check No Hardcoded Live JWT Tokens
    suspicious_jwts = re.findall(r'eyJhbGciOi[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+', col_str)
    for jwt in suspicious_jwts:
        assert "invalid" in jwt or "malformed" in jwt or "test" in jwt, f"Potentially hardcoded live JWT found: {jwt}"
    print("[PASS] 8. Zero hardcoded live JWT tokens detected.")
